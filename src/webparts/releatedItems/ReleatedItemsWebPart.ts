...
import * as strings from 'ReleatedItemsWebPartStrings';
import ReleatedItems from './components/ReleatedItems';
import { IReleatedItemsProps } from './components/IReleatedItemsProps';
import { SPListItem, SPList } from '@microsoft/sp-page-context';
import { PnPClientStorage } from '@pnp/common';

export interface IReleatedItemsWebPartProps {
  fieldInternalName: string;
}

export default class ReleatedItemsWebPart extends BaseClientSideWebPart<IReleatedItemsWebPartProps> {
...
const webpart=this;
      ...
      .then(_ => {
        ...
        return sp.web.lists.getById(list.id.toString()).items.getById(id).expand("File").get()
          .then((task) => {
            ...
            let relatedItems = JSON.parse(task["RelatedItems"]);
            debugger;
  ...
  public render(): void {
    ...
    const element: React.ReactElement<IReleatedItemsProps> = React.createElement(
      ReleatedItems,
      {
          ...
          groups: [
            {
              ...
              groupFields: [
                PropertyPaneTextField('fieldInternalName', {
